...
export function ProjectDetailTimeline({ commits, scanRecords }: {
  commits: GitCommitItem[]; scanRecords: ScanRecordItem[]
}) {
  const entries = [
    ...
    <div className="space-y-1">
      {entries.map((e, i) => (
        <div key={i} className="flex items-start gap-3 py-2 border-b last:border-0">
          ...
          <div><p className="text-sm">{e.summary}</p>
            <div className="flex gap-3 text-xs text-muted-foreground"><span>{new Date(e.date).toLocaleString()}</span>{e.detail && <span>{e.detail}</span>}</div>
          </div>
        </div>
      ))}
    </div>
